...
const valuation = ref<StockValuation | null>(null);
    ...
    <Spin :spinning="loading">
      <template v-if="stockInfo">
        ...
        <div class="mb-4 flex items-center gap-3">
          <a @click="goBack" class="cursor-pointer text-gray-400 hover:text-gray-600">
            ← 返回
          </a>
          ...
          <Tag :color="stockInfo.market === 'SH' ? 'red' : 'green'">
            {{ stockInfo.market === 'SH' ? '沪市' : stockInfo.market === 'SZ' ? '深市' : stockInfo.market }}
          </Tag>
              ...
              <Col :span="24">
                <Card title="估值指标" size="small">
                  <Row :gutter="16" v-if="valuation">
                    <Col :span="4">
                      <Statistic title="PE(TTM)" :value="formatNum(valuation.peTtm)" />
                    </Col>
                    <Col :span="4">
                      <Statistic title="PE(静态)" :value="formatNum(valuation.peStatic)" />
                    </Col>
                    <Col :span="4">
                    ...
                    <Col :span="4">
                      <Statistic title="总市值(亿)" :value="formatNum(valuation.totalMv)" />
                    </Col>
                    <Col :span="4">
                      <Statistic title="股息率" :value="formatPct(valuation.divYield)" />
                    </Col>
                    <Col :span="4" class="mt-3">
                      <Statistic title="PE分位" :value="formatPct(valuation.pePercentile)" />
                    </Col>
                    <Col :span="4" class="mt-3">
                      <Statistic title="PB分位" :value="formatPct(valuation.pbPercentile)" />
                    </Col>
                  </Row>
                  ...
                  <template v-if="reports.length > 0">
                    <Descriptions :column="1" size="small">
                      <Descriptions.Item label="报告期">
                        {{ reports[0]?.reportYear }} {{ reports[0]?.reportType ? (reportTypeMap[reports[0].reportType] || reports[0].reportType) : '' }}
                      </Descriptions.Item>
          ...
          <Tabs.TabPane key="finance" tab="财务指标">
            <Card size="small">
              <Table
                ...
                rowKey="reportYear"
              >
                <Table.Column title="报告期" dataIndex="reportYear" :width="80">
                  <template #default="{ record }">
                    {{ record.reportYear }}{{ reportTypeMap[record.reportType] || '' }}
                  </template>
                </Table.Column>
                <Table.Column title="营收(亿)" :width="120">
                  <template #default="{ record }">{{ formatYi(record.revenue) }}</template>
                </Table.Column>
                <Table.Column title="净利润(亿)" :width="120">
                  <template #default="{ record }">{{ formatYi(record.netProfit) }}</template>
                </Table.Column>
                <Table.Column title="EPS" :width="80">
                  <template #default="{ record }">{{ formatNum(record.eps) }}</template>
                </Table.Column>
                <Table.Column title="ROE" :width="80">
                  <template #default="{ record }">{{ formatPct(record.roe) }}</template>
                </Table.Column>
                <Table.Column title="毛利率" :width="80">
                  <template #default="{ record }">{{ formatPct(record.grossMargin) }}</template>
                </Table.Column>
                <Table.Column title="净利率" :width="80">
                  <template #default="{ record }">{{ formatPct(record.netMargin) }}</template>
                </Table.Column>
                <Table.Column title="资产负债率" :width="100">
                  <template #default="{ record }">{{ formatPct(record.debtRatio) }}</template>
                </Table.Column>
                <Table.Column title="营收同比" :width="80">
                  <template #default="{ record }">{{ formatPct(record.revenueYoy) }}</template>
                </Table.Column>
                <Table.Column title="净利同比" :width="80">
                  <template #default="{ record }">{{ formatPct(record.netProfitYoy) }}</template>
                </Table.Column>
              </Table>
          ...
          <Tabs.TabPane key="product" tab="主营产品">
            <Card size="small">
              <Table
                ...
                rowKey="productName"
              >
                <Table.Column title="产品/业务" dataIndex="productName" :width="200" />
                <Table.Column title="类型" dataIndex="productType" :width="80">
                  <template #default="{ record }">
                    <Tag>{{ productTypeMap[record.productType] || record.productType }}</Tag>
                  </template>
                </Table.Column>
                <Table.Column title="营收占比" :width="100">
                  <template #default="{ record }">{{ formatPct(record.revenueRatio) }}</template>
                </Table.Column>
                <Table.Column title="毛利率" :width="80">
                  <template #default="{ record }">{{ formatPct(record.grossMargin) }}</template>
                </Table.Column>
              ...
              <Col :span="24">
                <Card title="产业链关系" size="small">
                  <Table
                    ...
                    rowKey="id"
                  >
                    <Table.Column title="关系" dataIndex="relationType" :width="80">
                      <template #default="{ record }">
                    ...
                    <Table.Column title="关联公司" dataIndex="relatedCompany" :width="200" />
                    <Table.Column title="股票代码" dataIndex="relatedStockCode" :width="100">
                      <template #default="{ record }">
                        <a v-if="record.relatedStockCode" @click="router.push(`/finance/stock/${record.relatedStockCode}`)">
                          {{ record.relatedStockCode }}
                        </a>
                        <span v-else>-</span>
                      </template>
                    </Table.Column>
                    <Table.Column title="描述" dataIndex="relationDesc" />
                  </Table>
          ...
          <Tabs.TabPane key="shareholder" tab="股东信息">
            <Card size="small">
              <Table
                ...
                rowKey="id"
              >
                <Table.Column title="股东名称" dataIndex="holderName" :width="250" />
                <Table.Column title="类型" dataIndex="holderType" :width="80">
                  <template #default="{ record }">
                    <Tag>{{ holderTypeMap[record.holderType] || record.holderType }}</Tag>
                  </template>
                </Table.Column>
                <Table.Column title="持股数(万)" :width="130">
                  <template #default="{ record }">{{ formatWan(record.holdShares) }}</template>
                </Table.Column>
                <Table.Column title="持股比例" :width="80">
                  <template #default="{ record }">{{ formatPct(record.holdPct) }}</template>
                </Table.Column>
                <Table.Column title="变动(万)" :width="130">
                  <template #default="{ record }">
                    <span :class="(record.changeShares ?? 0) > 0 ? 'text-green-600' : (record.changeShares ?? 0) < 0 ? 'text-red-600' : ''">
                      {{ record.changeShares != null ? formatWan(record.changeShares) : '-' }}
                    </span>
                  </template>
                </Table.Column>
                <Table.Column title="报告期" dataIndex="reportDate" :width="110" />
              </Table>
          ...
          <Tabs.TabPane key="event" tab="动态事件">
            <Card size="small">
              <Timeline v-if="events.length > 0">
                <TimelineItem v-for="evt in events" :key="evt.eventDate + evt.eventType">
                  <div class="flex items-center gap-2">
                    <Tag :color="eventTypeMap[evt.eventType]?.color || 'default'">
